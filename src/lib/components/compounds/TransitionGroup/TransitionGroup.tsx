...
const TransitionGroup: Component = ({ children, style }) => {
    const chs = children as any[];

    const [rendered, setRendered] = useState(chs);
    const [exiting, setExiting] = useState(false);
    const [entering, setEntering] = useState(false);

    const [, max] = calculateMinMax(chs[0].props.transition);

    useEffect(() => {
        if (!exiting && !entering && !sameChildren(chs, rendered)) {
            let arr = [...rendered];
            //child thats in rendered but not in children
            for (const child of rendered.filter(
                (c: any) =>
                    !chs.find((child: any) => child.props.i === c.props.i)
            )) {
            ...
            for (const child of chs.filter(
                (c: any) =>
                    !rendered.find((child: any) => child.props.i === c.props.i)
            )) {
                const indexChildren = chs.findIndex(
